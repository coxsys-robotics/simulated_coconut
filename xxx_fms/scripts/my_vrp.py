...
adjacency_matrix = array(DISTANCES, dtype=[("cost", int)])
graph = from_numpy_matrix(adjacency_matrix, create_using=nx.DiGraph())

# Transform time matrix into DiGraph
#adjacency_matrix_time = array(TRAVEL_TIMES, dtype=[("time", int)])
#graph_time = from_numpy_matrix(adjacency_matrix_time, create_using=DiGraph()

#graph = nx.compose(graph, graph_time)
# Set time windows
#set_node_attributes(graph, values=TIME_WINDOWS_LOWER, name="lower")
#set_node_attributes(graph, values=TIME_WINDOWS_UPPER, name="upper")

# The demands are stored as node attributes
set_node_attributes(graph , values=DEMAND, name="demand")
#set_node_attributes(G, values=COLLECT, name="collect")

# The depot is relabeled as Source and Sink
graph = relabel_nodes(graph , {0: "Source", 17: "Sink"})